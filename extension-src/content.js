...
const EXT_PAYMENT_URL = "https://qpssaefptonzbpgcvtrq.supabase.co/functions/v1/process-extension-payment";
const PROXY_COMMAND_URL = "https://qpssaefptonzbpgcvtrq.supabase.co/functions/v1/proxy-command";
const REMOVE_WATERMARK_URL = "https://qpssaefptonzbpgcvtrq.supabase.co/functions/v1/remove-watermark";
const PUBLISH_PROJECT_URL = "https://qpssaefptonzbpgcvtrq.supabase.co/functions/v1/publish-project";
...
async function sendPromptNativeViaBackground(mensagem, modoPlano, attachedFilesSnapshot) {
  ...
  // Mesmo método da LovaSiri: as URLs das imagens são embutidas no texto do
  // prompt e attachedFiles vai sempre vazio (caso contrário o Lovable tenta
  // abrir as imagens como anexos nativos e falha).
  ...
  if (imageUrls.length) {
    const linksTxt = imageUrls.map((u, i) => 'Imagem ' + (i + 1) + ': ' + u).join('\n');
    const instrucao = imageUrls.length === 1
  ...
  const resp = await bgFetchRaw(
    'https://qpssaefptonzbpgcvtrq.supabase.co/functions/v1/send-lovable-prompt',
    {
      method: 'POST',
      headers: {
...
function injectShieldOverlay(){
  ...
  overlay.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    '</svg>' +
    '<span class="ql-shield-overlay-text" data-ts-brand="shield">\ud83d\udee1\ufe0f Protegido pelo ' + ((window.tsBrandName && window.tsBrandName()) || "Painel Lovable") + '</span>' +
...
const UPLOAD_IMAGE_EDGE_URL = 'https://qpssaefptonzbpgcvtrq.supabase.co/functions/v1/upload-prompt-image';
...
async function handleFilesAttach(files) {
  ...
  if (!token) { showCustomAlert('Erro', 'Token nao capturado. Abra o Lovable (lovable.dev) e faça uma ação no editor/chat para capturar o token, depois tente novamente.'); return; }
...
var VERSIONS_URL_POPUP = "https://qpssaefptonzbpgcvtrq.supabase.co/rest/v1/extension_versions?select=version,changelog,file_path,is_alert_active&order=created_at.desc&limit=1&is_alert_active=eq.true";
var USER_ROLES_URL_POPUP = "https://qpssaefptonzbpgcvtrq.supabase.co/rest/v1/user_roles?select=role";
...
async function checkForUpdatePopup() {
  try {
    ...
    if (latest.version !== CURRENT_EXT_VERSION_POPUP && latest.is_alert_active) {
      ...
      if (banner) {
        var dlUrl = latest.file_path ? "https://qpssaefptonzbpgcvtrq.supabase.co/storage/v1/object/public/extension-releases/" + latest.file_path : null;
...
async function checkResellerRolePopup() {
  try {
    ...
    var licData = await bgFetch("https://qpssaefptonzbpgcvtrq.supabase.co/rest/v1/ts_licenses?select=user_id&license_key=eq." + encodeURIComponent(storageData.ql_license_key) + "&limit=1", { method: "GET", headers: { apikey: SUPABASE_ANON_KEY } });
...
function injectNativeChatOverlay() {
  ...
  if (!document.getElementById("ql-native-badge")) {
    ...
    badge.innerHTML = '<span data-ts-brand="name">' + ((window.tsBrandName && window.tsBrandName()) || "Painel Lovable") + '</span>';
...
(function lovableDoneSoundFeature(){
  try {
    ...
    var SOUND_URLS = {
      send: "https://qpssaefptonzbpgcvtrq.supabase.co/storage/v1/object/public/sounds/send.mp3",
      done: "https://qpssaefptonzbpgcvtrq.supabase.co/storage/v1/object/public/sounds/done.mp3"
    };
