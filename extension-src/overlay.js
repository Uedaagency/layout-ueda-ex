...
(function () {
  ...
  function getTsHeaderLabel() {
    return "⚡ Enviado por UEDA EX";
  }
  const TS_HEADER_LABEL_RE = /^\s*(?:lov\s*\d+(?:\.\d+)?|fix(?:\s+build)?\s*error|corrigir\s+erro(?:\s+de\s+build)?|enviado\s*por\s*[\u26A1\u2728\u{1F4AC}\u{1F680}\s]*.+)\s*$/iu;
