...
(function () {
  ...
  function injectGlobalStyles() {
    ...
    style.textContent = `
      ...
      body.ts-native-chat-active .${COMPOSER_WRAP_CLASS} {
        ...
        box-shadow: var(--ts-primary-glow), 0 0 24px rgba(124, 90, 255, 0.15) !important;
  ...
  function updateNativeBadge() {
    ...
    if (!existing) { badge.id = "ts-native-badge"; badge.textContent = "TS ATIVO"; document.body.appendChild(badge); }
  ...
  const LICON = {
    ...
    badgeX:     '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="m7 21-4.3-4.3c-1-1-1-2.5 0-3.4l9.6-9.6c1-1 2.5-1 3.4 0l5.6 5.6c1 1 1 2.5 0 3.4L13 21"/><path d="M22 21H7"/><path d="m5 11 9 9"/></svg>',
    ...
    refresh:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>',
    ...
    if (action === "sidebar") {
      ...
      try {
        chrome.runtime.sendMessage({ action: "checkUpdate" }, (r) => {
          if ((chrome.runtime && chrome.runtime.lastError) || !r || !r.ok) {
            showStatus("✗ Não foi possível verificar agora. Tente novamente.", "error");
            return;
          }
          if (!r.hasUpdate) {
            showStatus("✓ Você já está na versão mais recente (v" + (r.installedVersion || "-") + ")", "success");
            return;
          }
          showStatus("⬇️ Nova versão v" + (r.latestVersion || "") + " — baixando…");
          chrome.runtime.sendMessage({ action: "downloadUpdateZip", url: r.zipUrl, version: r.latestVersion, filename: r.filename }, (dl) => {
            if (dl && dl.ok) showStatus("✅ Download iniciado (v" + (dl.version || r.latestVersion || "") + "). Veja a pasta Downloads.", "success");
            else showStatus("✗ Falha ao baixar" + (dl && dl.error ? ": " + dl.error : "") + ".", "error");
          });
        });
      } catch (_) { showStatus("✗ Falha ao verificar a atualização.", "error"); }
  ...
  function getTsHeaderLabel() {
    return "⚡ Enviado por UEDA";
  }
  const TS_HEADER_LABEL_RE = /^\s*(?:lov\s*\d+(?:\.\d+)?|fix(?:\s+build)?\s*error|corrigir\s+erro(?:\s+de\s+build)?|try\s+to\s+fix\s+seo\s+issue.*|(?:fix\s+)?seo\s+issue.*|meta\s*description.*|corrigir\s+(?:problema|erro)s?\s+de\s+seo.*|edi[çc][ãa]o\s+visual|visual\s+edit(?:s)?|security\s*scan|verifica[çc][ãa]o\s+de\s+seguran[çc]a|enviado\s*por\s*[\u26A1\u2728\u{1F4AC}\u{1F680}\s]*.+)\s*$/iu;
